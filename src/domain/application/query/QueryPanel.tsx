...
export default class QueryPanel extends React.Component<any, any> {
	...
	translate() {
		Utils.postReq('http://localhost:8098/translate', {
	...
	render() {
		...
		return (
			<div>
				<div>
					<label htmlFor='sel-Entity'>Search for: &nbsp;</label>
					<select id="sel-Entity"
						key="sel-Entity" onChange={this.onSelChange}
						value={this.state.selEntity}
					>
						<option key="defEnt" value="">
							--Select--
						</option>
						{this.props.entities.map((item: any) => (
